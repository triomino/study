...
denote S as the student network to be trained. Same copies of data are sent to T and S respectively.
...
Equation ~\eqref{eqn:kd_loss} gives the distillation loss, where $\textrm{H}(p, q)$ is cross entroy loss function and $t$ is temperature of softmax function. Equation ~\eqref{eqn:gt_loss} gives the
ground truth loss of classification. The final training loss is a weighted sum of these two terms.

\begin{equation}
\label{eqn:kd_loss}
L=\sum\limits_{(x_i, y_i)\in \mathcal D}{\textrm{H}(\textrm{softmax}(\frac{f_T(x_i)}{t}), \textrm{softmax}(\frac{f_S(x_i; \theta)}{t}))}
\end{equation}

\begin{equation}
\label{eqn:gt_loss}
L_{GT}=\sum\limits_{(x_i, y_i)\in \mathcal D}{\textrm{H}(\textrm{one-hot}(y_i),\textrm{softmax}(f_S(x_i; \theta)))}
\end{equation}
...
Note that inner layers can be a part the knowledge.~\cite{romero2014fitnets}~\cite{yao2020knowledge}~\cite{DBLP:journals/corr/ZagoruykoK16a}
conetent

...
def rnn_step_backward(dnext_h, cache):
    ...
    daff = (1 - next_h * next_h) * dnext_h
    dWh = prev_h.T.dot(daff)
    dprev_h = daff.dot(Wh.T)
    dWx = x.T.dot(daff)
    dx = daff.dot(Wx.T)
    db = np.sum(daff, axis = 0)
...
def rnn_backward(dh, cache):
    ...
    for i in reversed(range(T)):
        dxi, dprev_h, dWxi, dWhi, dbi =  rnn_step_backward(dh[:, i, :] + dprev_h, cache[i])
        if i == T-1:
            dWx = dWxi
            dWh = dWhi
            db = dbi
            dx = np.zeros((dxi.shape[0], T, dxi.shape[1]))
        else:
            dWx += dWxi
            dWh += dWhi
            db += dbi
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    next_h = o * tanhc
    cache = (i, f, o, g, tanhc, A, Wx, Wh, x, prev_h, prev_c)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    i, f, o, g, tanhc, A, Wx, Wh, x, prev_h, prev_c = cache
    dtanhc = dnext_h * o
    dnext_c = dnext_c + dtanhc * (1 - tanhc ** 2)
    dprev_c = dnext_c * f
    di = dnext_c * g
    df = dnext_c * prev_c
    dg = dnext_c * i
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for i in range(T):
        h0, c, _ = lstm_step_forward(x[:, i, :], h0, c, Wx, Wh, b)
        h[:, i, :] = h0
        cache.append(_)

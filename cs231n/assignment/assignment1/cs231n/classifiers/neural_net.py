...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        loss = -np.sum(np.log(prob[np.arange(N), y]))/N + reg * (np.sum(W1*W1) + np.sum(W2*W2)) 
        ...
        grads['W2'] = hidden_layer.T.dot(prob) / N + 2.0 * reg * W2
        grads['b2'] = np.ones(N).dot(prob) / N
        dhidden = prob.dot(W2.T)
        dhidden[hidden_layer <= 0] = 0
        grads['W1'] = X.T.dot(dhidden) / N + 2.0 * reg * W1
